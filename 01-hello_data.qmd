...
`dataframe` 顾名思义，也叫方形数据（Rectangular Data）。Python（pandas包）和R都将这种数据称为dataframe，也是最常见的数据对象。在R中，还有`tibble`和`data.table`这类`dataframe`的变种，本质上还是方形数据，只不过用来处理的包和风格不太相同。
